...
from tensorflow.keras.layers import Bidirectional, LSTM, Dropout, Embedding, Concatenate, GlobalAveragePooling1D
...
def build_model(args):
    ...
    trace_in = Input(shape=(num_samples, 1), name="trace")

    # CNN front-end
    x = Conv1D(32, 9, padding="same", activation="relu")(trace_in)
    x = BatchNormalization()(x)
    x = MaxPooling1D(4)(x)
    ...
    shared = Dense(256, activation='relu', name='shared_fc')(x)
    shared = BatchNormalization()(shared)

    # 8 outputs (byte_00..byte_07) — names preserved for your plotting
    outs = []
    for i in range(8):
        z = Dense(1024, activation='relu', name=f'byte_{i:02d}_fc1')(shared)
        z = BatchNormalization()(z)
        outs.append(Dense(256, activation="softmax", name=f'byte_{i:02d}_output')(z))

    return Model(inputs=trace_in, outputs=outs, name='CNN_BiLSTM_8bytes')
